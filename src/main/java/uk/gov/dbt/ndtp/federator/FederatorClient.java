...
public class FederatorClient {
  ...
  private static ConsumerConfigService createConfigService() {
    ...
    final ManagementNodeDataHandler handler =
        new ManagementNodeDataHandler(httpClient, mapper, tokenService);
  ...
  private static HttpClient createDefaultClient() {
    return HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(HTTP_TIMEOUT)).build();
  }
  ...
  private void registerDynamicJob() {
    String scheduleExpression = configService.getConsumerConfiguration().getScheduleExpression();
    if (scheduleExpression == null) {
      scheduleExpression = JobsConstants.DEFAULT_DURATION_EVERY_HOUR;
      LOGGER.warn(
          "No schedule expression found, using default value of {}", JobsConstants.DEFAULT_DURATION_EVERY_HOUR);
    }
