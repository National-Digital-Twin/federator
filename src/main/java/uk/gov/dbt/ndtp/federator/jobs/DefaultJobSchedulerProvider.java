...
public final class DefaultJobSchedulerProvider implements JobSchedulerProvider {
     ...
     */
    public static DefaultJobSchedulerProvider withDependencies(final JobScheduler scheduler,
                                                               final AbstractStorageProvider storage,
                                                               final RecurringJobsAccess jobsAccess) {
    ...
    public void ensureStarted() {
        ...
        synchronized (lifecycleLock) {
            ...
            // Only memory supported by default. If unsupported, fall back to in-memory to keep tests and runtime stable.
    ...
    private RecurringJobsAccess defaultAccess() {
        return new RecurringJobsAccess() {
            ...
            public java.util.Set<String> ids(AbstractStorageProvider sp) {
                return sp.getRecurringJobs().stream().map(RecurringJob::getId)
    ...
    public JobScheduler getJobScheduler(){
        if(started)
            ...
            throw new IllegalStateException("JobScheduler not started");
    }
    ...
    public  void  registerJob(Job job, JobParams params){
        ...
        recurringJobBuilder.withInterval(params.getDuration())
        ...
        jobScheduler.createRecurrently(
                recurringJobBuilder);
        log.info("Registered recurring job id={} name={} interval={} retries={}", params.getJobId(), params.getJobName(), params.getDuration(), params.getAmountOfRetries());
     ...
     */
    private Map<String, JobParams> buildRequestedJobMapForNode(final String managementNodeId,
        ...
        return requests.stream()
                ...
                .collect(java.util.stream.Collectors.toMap(JobParams::getJobId, java.util.function.Function.identity(), (a, b) -> b));
    }
