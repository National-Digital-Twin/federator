...
/**
 * Job handler for dynamic configuration updates from Management Node.
 * Fetches producer configurations from the Management Node
 * and creates GRPC jobs for each producer/product combination.
 */
@Slf4j
public class ClientDynamicConfigJob implements Job {
    ...
    private static final String LOG_START =
            "Fetching configs from Management Node [nodeId={}]";
    private static final String LOG_ERROR =
            "Config fetch failed [nodeId={}, error={}]";
    private static final String LOG_RELOAD =
            "Reloading jobs [count={}, nodeId={}]";
    private static final String LOG_INVALID =
            "Invalid producer [name='{}', host='{}', port={}]";
    ...
    private static final String LOG_SUCCESS =
            "Config fetched [producers={}, nodeId={}]";
    private static final String LOG_NO_CONFIG =
            "No config data received [nodeId={}]";
    private static final String LOG_JOB =
            "Job created [id={}, topic={}, host={}, port={}]";
    private static final String LOG_NULL = "Null producer skipped";
    private static final String LOG_CONNECTION =
            "Connection [server={}, host={}, port={}, tls={}]";
    private static final String LOG_ERROR_UNEXP =
            "Unexpected error [nodeId={}, error={}]";
    ...
    /**
     * Initializes the static service reference.
     * Must be called before JobRunr executes jobs.
     *
    ...
    /**
     * Creates a new job using static service reference.
     * Used by JobRunr when deserializing jobs.
     */
    ...
    private void reloadJobs(
        ...
        final List<RecurrentJobRequest> requests =
                buildJobRequests(config, nodeId);
    ...
    private List<RecurrentJobRequest> buildJobRequests(
            final ProducerConfigDTO config,
            final String nodeId) {
        final List<RecurrentJobRequest> requests =
                new ArrayList<>();
        ...
        if (config.getProducers() != null) {
            log.info(LOG_SUCCESS,
                    config.getProducers().size(), nodeId);
    ...
    private void logInvalidProducer(final ProducerDTO producer) {
        ...
        log.warn(LOG_INVALID,
                producer.getName(),
                producer.getHost(),
                producer.getPort());
    }

    private ConnectionProperties buildConnection(
            final ProducerDTO producer) {
    ...
    private int getPort(final ProducerDTO producer) {
        return producer.getPort() != null
                ? producer.getPort().intValue()
                : ConnectionProperties.DEFAULT_PORT;
    }

    private boolean getTls(final ProducerDTO producer) {
        return producer.getTls() != null
                ? producer.getTls()
                : ConnectionProperties.DEFAULT_TLS;
    }
    ...
    private void addJobRequest(
            ...
            final String nodeId,
            final List<RecurrentJobRequest> requests) {
        final RecurrentJobRequest request =
                createJobRequest(product, conn, nodeId);
    ...
    private RecurrentJobRequest createJobRequest(
            ...
            final ConnectionProperties conn,
            final String nodeId) {
        final ClientGRPCJobParams params = buildJobParams(
                product, conn, nodeId);
    ...
    private ClientGRPCJobParams buildJobParams(
            ...
            final ConnectionProperties conn,
            final String nodeId) {
        final ClientGRPCJobParams params = new ClientGRPCJobParams(
                product.getTopic(), conn, nodeId);
        params.setJobName(conn.serverName());
        params.setJobId(conn.serverName() + SEPARATOR +
                product.getTopic()); // FIX: Was duplicating serverName
