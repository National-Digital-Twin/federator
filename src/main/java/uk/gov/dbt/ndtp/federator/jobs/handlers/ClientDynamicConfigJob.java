...
public class ClientDynamicConfigJob implements Job {
  ...
  private List<RecurrentJobRequest> buildJobRequests(
      final ProducerConfigDTO config, final String nodeId) {
  ...
  private ConnectionProperties buildConnection(final ProducerDTO producer) {
    ...
    return new ConnectionProperties(
        producer.getIdpClientId(), NA_VALUE, serverName, host, port, tls);
  }
  ...
  private RecurrentJobRequest createJobRequest(
    ...
    return RecurrentJobRequest.builder().jobParams(params).job(new ClientGRPCJob(params)).build();
  }
  ...
  private ClientGRPCJobParams buildJobParams(
    ...
    params.setJobId(
        conn.serverName() + SEPARATOR + product.getTopic()); // FIX: Was duplicating serverName
