...
public class ClientDynamicConfigJob implements Job {
   ...
   */
  public ClientDynamicConfigJob(ManagementNodeService managementNodeService,
                                JobSchedulerProvider schedulerProvider) {
  ...
  public void run(JobParams value) {
          ...
          productsByProducerName.forEach(
              topic -> {
                ...
                RecurrentJobRequest request =
                    RecurrentJobRequest.builder().jobParams(clientGRPCJobParams).job(job).build();
    ...
    private static ClientGRPCJobParams getClientGRPCJobParams(ConnectionProperties config, ProductDTO topic, String managementNodeId) {
        ClientGRPCJobParams clientGRPCJobParams =
            new ClientGRPCJobParams(topic.getTopic(), config, managementNodeId);
