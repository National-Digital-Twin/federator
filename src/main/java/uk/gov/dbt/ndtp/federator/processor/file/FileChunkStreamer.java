...
public class FileChunkStreamer {
    ...
    public void stream(
            long fileSequenceId, FileFetchRequest fileFetchRequest, StreamObservable<FileChunk> streamObserver) {
        File file = new File(fileFetchRequest.path());

        try (FileFetchResult fetchResult = fetch(fileFetchRequest)) {
    ...
    private FileFetchResult fetch(FileFetchRequest request) {
        FileFetcher fileFetcher = FileFetcherFactory.getFetcher(request.sourceType());
        return fileFetcher.fetch(request);
    }
