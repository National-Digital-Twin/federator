...
class FileChunkStreamerTest {
    ...
    void test_stream_success_sendsDataChunksAndLastChunkChecksum() throws Exception {
        ...
        FileFetcher mockFetcher = mock(FileFetcher.class);
        when(mockFetcher.fetch(any())).thenReturn(new FileFetchResult(bais, fileSize));

        try (MockedStatic<FileFetcherFactory> mockedFactory = Mockito.mockStatic(FileFetcherFactory.class)) {
            mockedFactory.when(() -> FileFetcherFactory.getFetcher(any())).thenReturn(mockFetcher);

            FileChunkStreamer cut = new FileChunkStreamer(chunkSize);
            CapturingObserver observer = new CapturingObserver();
            FileFetchRequest request = new FileFetchRequest(SourceType.LOCAL, null, "test-file.txt");
    ...
    void test_stream_error_propagatesAsInternalStatus() {
        FileFetcher mockFetcher = mock(FileFetcher.class);
        when(mockFetcher.fetch(any())).thenThrow(new RuntimeException("boom"));
        ...
        try (MockedStatic<FileFetcherFactory> mockedFactory = Mockito.mockStatic(FileFetcherFactory.class)) {
            ...
            CapturingObserver observer = new CapturingObserver();
            FileFetchRequest request = new FileFetchRequest(SourceType.LOCAL, null, "bad.txt");
